<template>
  <div class="fr jsd">
    <div class="content" >
      ...
      <div class="user ac fc jc" :style="{'left': `${blackX}px`, 'top': `${blackY}px`}">
        <div :class="'pipeline-' + pipelineDirection"></div>
        <span>black</span>
      </div>
      <!-- <div class="user ac fc jc red" :style="{'left': `${redX}px`, 'top': `${redY}px`, 'background': red}">
        <div :class="'red pipeline-' + pipelineDirection"></div>
        <span>red</span>
      </div> -->
...
<script>
import { publicControlList } from '../assets/config/data/data'
import { soloControlList, customList, keyCodeType } from '../assets/config/data/double-game'
...
export default {
  ...
  methods: {
    ...
    move (type) {
      ...
      let direction = (type === 'left' || type === 'right') ? 'blackX' : 'blackY'
      // 屏幕的宽高
      const { width, height } = this // 40px user的宽高
      const value = +`${symbol}${this.controls.disdance}`
      this[direction] += value

      if (direction === 'blackX' && (this[direction] > width - 80  || this[direction] < 20)) {
        this[direction] -= value
        return
      }
      if (direction === 'blackY' && (this[direction] > height - 60  || this[direction] < 20)) {
        this[direction] -= value
        return
      }
      const { blackX: userX, blackY: userY } = this
      // 是否遇到障碍物
      ...
      if (!this.timer) {
        this.timer = setInterval(() => {
          if (this.isAttack && this.bulletsFrequency) {
            ...
            const { blackX: left, blackY: top } = this
            let params = {
              direction: pd,
              type: 'user'
            }
